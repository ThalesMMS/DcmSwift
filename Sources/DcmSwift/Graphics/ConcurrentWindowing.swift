...
internal func applyWindowTo8Concurrent(src: [UInt8], width: Int, height: Int, winMin: Int, winMax: Int, into dst: inout [UInt8]) async throws {
    ...
    if numPixels > 4096 {
        ...
        var negSubtract = -subtract
        vDSP_vsadd(floatSrc, 1, &negSubtract, &floatSrc, 1, vDSP_Length(numPixels))
...
@available(macOS 10.15, iOS 13, *)
internal func applyLUTTo16Concurrent(
    src: [UInt16],
    width: Int,
    height: Int,
    lut: [UInt8],
    ...
    guard lut.count >= 65_536 else {
        throw WindowingError.invalidLUTSize(expected: 65_536, actual: lut.count)
    }

    // Threshold empírico; ajuste via benchmark
    if numPixels > 2_000_000 {
        let threads = max(1, ProcessInfo.processInfo.activeProcessorCount)
        let chunkSize = (numPixels + threads - 1) / threads

        src.withUnsafeBufferPointer { inPtr in
            lut.withUnsafeBufferPointer { lutPtr in
                dst.withUnsafeMutableBufferPointer { outPtr in
        ...
        // Caminho single-thread simples e rápido
        var i = 0
        let end = numPixels & ~3
        while i < end {
            dst[i]   = lut[Int(src[i])]
